...
discord_links = re.findall(r'<a class="secondary-button normative-source" href="([^"]+)" target="_blank" rel="noopener noreferrer">', normative)
require(len(discord_links) == 10 and len(set(discord_links)) == 9, "Original Discord links are incomplete")
require(all(link.startswith("https://discord.com/") for link in discord_links), "An original link is not HTTPS Discord")
