"""PostgreSQL-backed storage for persistent Elo/MMR and monthly ranking results."""
    ...
    with _connect_writer(resolved_path, application_name=application_name) as connection:
        connection.executemany(
            ...
            ON CONFLICT(scope_key, external_match_id, stable_player_key) DO UPDATE SET
                ...
                fact_schema_version = excluded.fact_schema_version,
                source_input_version = excluded.source_input_version,
                model_version = excluded.model_version,
                formula_version = excluded.formula_version,
                contract_version = excluded.contract_version,
                match_valid = excluded.match_valid,
                quality_factor = excluded.quality_factor,
                quality_bucket = excluded.quality_bucket,
                role_bucket = excluded.role_bucket,
                role_bucket_mode = excluded.role_bucket_mode,
                outcome_score = excluded.outcome_score,
                combat_index = excluded.combat_index,
                objective_index = excluded.objective_index,
                objective_index_mode = excluded.objective_index_mode,
                utility_index = excluded.utility_index,
                utility_index_mode = excluded.utility_index_mode,
                leadership_index = excluded.leadership_index,
                leadership_index_mode = excluded.leadership_index_mode,
                discipline_index = excluded.discipline_index,
                discipline_index_mode = excluded.discipline_index_mode,
                ...
                strength_of_schedule_match = excluded.strength_of_schedule_match,
                team_outcome = excluded.team_outcome,
                own_team_average_mmr = excluded.own_team_average_mmr,
                enemy_team_average_mmr = excluded.enemy_team_average_mmr,
                expected_result = excluded.expected_result,
                actual_result = excluded.actual_result,
                won_score = excluded.won_score,
                margin_boost = excluded.margin_boost,
                outcome_adjusted = excluded.outcome_adjusted,
                match_impact = excluded.match_impact,
                combat_contribution = excluded.combat_contribution,
                objective_contribution = excluded.objective_contribution,
                utility_contribution = excluded.utility_contribution,
                survival_discipline_contribution = excluded.survival_discipline_contribution,
                exact_component_contribution = excluded.exact_component_contribution,
                proxy_component_contribution = excluded.proxy_component_contribution,
                normalization_bucket_key = excluded.normalization_bucket_key,
                normalization_fallback_reason = excluded.normalization_fallback_reason,
                elo_core_delta = excluded.elo_core_delta,
                performance_modifier_delta = excluded.performance_modifier_delta,
                proxy_modifier_delta = excluded.proxy_modifier_delta,
                canonical_fact_capability_status = excluded.canonical_fact_capability_status,
                identity_capability_status = excluded.identity_capability_status,
                match_duration_seconds = excluded.match_duration_seconds,
                duration_source_status = excluded.duration_source_status,
                duration_bucket = excluded.duration_bucket,
                player_count = excluded.player_count,
                objective_score_proxy = excluded.objective_score_proxy,
                objective_score_proxy_mode = excluded.objective_score_proxy_mode,
                kills_per_minute = excluded.kills_per_minute,
                combat_per_minute = excluded.combat_per_minute,
                support_per_minute = excluded.support_per_minute,
                objective_proxy_per_minute = excluded.objective_proxy_per_minute,
                participation_bucket = excluded.participation_bucket,
                ...
                SELECT
                    ...
                    CASE
                        WHEN historical_matches.started_at IS NOT NULL
                         AND historical_matches.ended_at IS NOT NULL
                         AND historical_matches.ended_at >= historical_matches.started_at
                            THEN CAST(EXTRACT(EPOCH FROM (historical_matches.ended_at - historical_matches.started_at)) AS INTEGER)
                    ...
                    CASE
                        WHEN historical_matches.started_at IS NOT NULL
                         AND historical_matches.ended_at IS NOT NULL
                         AND historical_matches.ended_at >= historical_matches.started_at THEN 'exact'
...
def _percentile(values: list[float], ratio: float) -> float:
    ...
    connection.execute(
        ...
        SELECT
            ...
            CASE
                WHEN pel.event_type = 'player_teamkill_summary' OR pel.is_teamkill = TRUE THEN 'friendly_fire_summary'
            ...
            CASE WHEN pel.event_type = 'player_teamkill_summary' OR pel.is_teamkill = TRUE THEN 1 ELSE 0 END AS is_friendly_fire,
            0 AS is_redeploy,
    ...
    try:
        with _connect_readonly(resolved_path) as connection:
            ...
            rows = connection.execute(
                ...
                ORDER BY monthly_rank_score DESC, current_mmr DESC, LOWER(player_name) ASC, player_name ASC
                LIMIT ?
                """,
                params,
            ).fetchall()
